...
public class Main {
	...
	private static final String COMMENT_VOTES_TABLE_DEF = 
			...
			+ "downvotes counter"
			+ ")";
//        private static final String COMMENT_SEARCHQuery = 
//                "SELECT * FROM "+Constants.KEYSPACE+".comments where id = 66187530-3a37-11e6-984c-492e4e95e7df";
//        public static void executeSearchQuery(){
//            Session conn = CassandraConnection.getSession();
//            long time = System.currentTimeMillis();
//            ResultSet rs= conn.execute(COMMENT_SEARCHQuery);
//            time = System.currentTimeMillis()-time;
//            log.info("Spent Time: "+ time);
//            for(Row r: rs.all()){
//                log.info(r.getUUID("id").toString()+"-"+r.getString("commenter")+" : "+r.getString("title")+"="+r.getString("content"));
//            }
//            conn.close();
//        }
	...
	private static void insertData(){
		...
		try (SessionWrapper sessionWrapper = new SessionWrapper()){
			for (String[] blag: blogs){
				...
				// insert 10-20 posts
				insertPosts(sessionWrapper, blog.getId(), getInt(9, 10));
			}
		}
            time = System.currentTimeMillis()-time;
            log.info("Inserting Time is: "+time/1000+ " s");
	}
	...
	private static void insertPosts(SessionWrapper sessionWrapper, UUID blogId, int numPosts){
		for (int i=0; i<numPosts; i++){
			...
			insertComments(sessionWrapper, post, getInt(790,800 ));
		}
	}
	...
	public static void main(String[] args) {
		try {
//                    if(true){ printPostsAndComments(getRandomBlog()); return;}
			setup();
